...
| 4 | **S-track** — GPU season animation (folds in G5i) | SA0–SA4 | ◑ **SA0–SA2 shipped** (targeting core + live state machine + the sprung open-season cloud behind `?gpuseason=1`, real-GPU verified); **SA3** (hybrid GPU frontier) + **SA4** (polish + graduate the flag) pending | agent-doable end-to-end — **`scripts/snap-realgpu.js`** (Playwright headed real GPU) verifies the visible glide, no human MANUAL needed |
