...
//
// SPRING-TWIN CHANGE: we no longer read the integer counters hr[]/sb[]. We read the SMOOTHED
// position pos[] that spring.wgsl produced (still in HR/SB units, so the NDC mapping is unchanged),
// and onFront[] is now written by skyline.wgsl on the GPU (not uploaded from the CPU frontier).
//
...
//   1 pos      storage   per-player smoothed position vec2(HR,SB)  (written by spring.wgsl)
//   2 debut    storage   per-player debut year  (drives the era colour; static)
//   3 onFront  storage   per-player frontier flag (written by skyline.wgsl)
struct Params { maxX: f32, maxY: f32, ptSize: f32, aspect: f32 };
@group(0) @binding(0) var<uniform> P: Params;
@group(0) @binding(1) var<storage, read> pos: array<vec2<f32>>;
@group(0) @binding(2) var<storage, read> debut: array<u32>;
...
fn vs(@builtin(vertex_index) vi: u32, @builtin(instance_index) ii: u32) -> VSOut {
    ...
    let pp = pos[idx];                  // smoothed position (HR, SB), from spring.wgsl
    // Map the player's position to clip space. NDC is [-1,1]; we target [-0.95, 0.95]
    ...
    let x = (pp.x / max(P.maxX, 1.0)) * 1.9 - 0.95;   // max(...,1) guards divide-by-zero before any events
    let y = (pp.y / max(P.maxY, 1.0)) * 1.9 - 0.95;
