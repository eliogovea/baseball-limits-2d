...
- [x] **Player spotlight card** *(shipped 2a7111a, refined 0e8c0d5)* — pinning a single player opens a card with monogram, career span/handedness, frontier-season count, % of frontier area owned (summed hypervolume contribution), a 2-D career-trajectory mini-plot (the player's seasons in both active axes, frontier seasons marked), and the record seasons. One card per pinned player (in the player's trail color); anchored opposite the frontier and draggable by its header. From the Claude Design upgrades.
